...
const REDIRECT_URI = process.env.DISCORD_REDIRECT_URI || 'https://zyroapi.shardweb.app/api/auth/discord/callback';
...
export const discordLogin = (req: Request, res: Response) => {
    const url = `https://discord.com/api/oauth2/authorize?client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI!)}&response_type=code&scope=identify%20email%20guilds%20guilds.join%20guilds.members.read`;
    res.redirect(url);
};
